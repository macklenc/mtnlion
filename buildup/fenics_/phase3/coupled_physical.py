...
def run(start_time, dt, stop_time, return_comsol=False):
    ...
    phie_lhs, phie_rhs1, phie_rhs2 = equations.phie(
        j, ce, phie, v_phie, kappa_eff, kappa_Deff, **cmn.fenics_params, **cmn.fenics_consts
    )
    ...
    if start_time < dt:
        ...
        utilities.assign_functions(
            [comsol_phis(0), comsol_phie(0), comsol_j(0)], [phis_c_, phie_c_, j_c], domain.V, Ellipsis
        )
    else:
        utilities.assign_functions([comsol_ce(start_time)], [ce_c_1], domain.V, ...)
        utilities.assign_functions(
            [comsol_phis(start_time - dt), comsol_phie(start_time - dt), comsol_j(start_time - dt)],
            [phis_c_, phie_c_, j_c],
            domain.V,
            ...,
        )
    ...
    if return_comsol:
        return (
            utilities.interp_time(time, phis_sol),
            utilities.interp_time(time, phie_sol),
            utilities.interp_time(time, j_sol),
            utilities.interp_time(time, ce_sol),
            comsol,
        )
    else:
        return (
...
def main():
    ...
    utilities.report(
        comsol.neg, plot_time, j_sol(plot_time)[:, comsol.neg_ind], comsol_j(plot_time)[:, comsol.neg_ind], "$j^{neg}$"
    )
    utilities.save_plot(__file__, "plots/coupled_physical/compare_j_neg_newton.png")
    plt.show()
    utilities.report(
        comsol.pos, plot_time, j_sol(plot_time)[:, comsol.pos_ind], comsol_j(plot_time)[:, comsol.pos_ind], "$j^{pos}$"
    )

...
def run(time, dt, return_comsol=False):
    ...
    for k, t in enumerate(time):
        ...
        utilities.assign_functions([comsol_ce(t), comsol_cse(t), comsol_j(t)], [ce_c, cse_c, j_c], domain.V, ...)
...
def main():
    ...
    utilities.report(
        comsol.neg, time, j_sol(plot_time)[:, comsol.neg_ind], comsol_j(plot_time)[:, comsol.neg_ind], "$j^{neg}$"
    )
    utilities.save_plot(__file__, "plots/coupled_phie_phis_j/compare_j_neg_newton.png")
    plt.show()
    utilities.report(
        comsol.pos, time, j_sol(plot_time)[:, comsol.pos_ind], comsol_j(plot_time)[:, comsol.pos_ind], "$j^{pos}$"
    )

...
def main():
    ...
    for i in range(len(time_in)):
        ...
        Feq = phis_form(neumann=fem.Constant(Iapp[i]) / Acell) + \
              ...
              fem.inner(u_ce, v_ce) * dx + fem.inner(u_cse, v_cse) * dx + fem.inner(u_phis, v_phis) * dx(1)
    ...
    plt.show()
    utilities.report(comsol.pos, time_in, phis_array[:, comsol.pos_ind],
    ...
    plt.show()
    utilities.report(comsol.pos, time_in, j_array[:, comsol.pos_ind],

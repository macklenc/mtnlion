...
def main():
    ...
    phis_form = partial(
        equations.phis, j, phis_f, v, domain.dx((0, 2)), **cmn.fenics_params, **cmn.fenics_consts, ds=domain.ds(4)
    )
    ...
    utilities.report(
        comsol.neg, time_in, u_array[:, comsol.neg_ind], comsol.data.phis[:, comsol.neg_ind][1::2], "$\Phi_s^{neg}$"
    )
    plt.show()
    utilities.report(
        comsol.pos, time_in, u_array[:, comsol.pos_ind], comsol.data.phis[:, comsol.pos_ind][1::2], "$\Phi_s^{pos}$"
    )
    plt.show()

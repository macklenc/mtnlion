...
def fix_boundaries(
    mesh: np.ndarray, data: np.ndarray, boundaries: Union[float, List[int], np.ndarray]
...
def format_2d_data(
    raw_data: Dict[str, np.ndarray], boundaries: Union[float, List[int]]
...
def format_pseudo_dim(
    raw_data: Dict[str, np.ndarray], boundaries: Union[float, List[int]], shuffle: Callable = lambda x: range(len(x))
) -> Union[Dict[str, np.ndarray], None]:
    """
    ...
    try:
        ...
        mesh_dict = {
            "time_mesh": raw_data["time_mesh"],
            "pseudo_mesh": np.array(raw_data["pseudo_mesh"])[indices],

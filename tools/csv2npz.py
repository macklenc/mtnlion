...
def main(
    ...
    output: Union[click.utils.LazyFile, str],
    dt: List[float],
    loglevel: Union[None, int],
    bound: Union[None, str],
) -> Union[None, int]:
    """
    ...
    if "time_mesh" not in file_data:
        try:
            ...
            logger.critical(
                "Either a dt option provided with start and stop time (inclusive) or a csv defining the "
                "time mesh is required",
                exc_info=True,
            )
            raise ex
